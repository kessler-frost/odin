...
**Status:** Approved design (brainstorm complete 2026-06-21). Implementation begins on branch `odin`.
...
ODIN is a Mac-native, visualizable, intelligence-backed orchestration system. Mental model: **"Railway, but local-first on a single Mac, with an AI operator."** You drop applications and infrastructure onto a visual canvas; an AI places and configures them; odin's own control loop runs and supervises them *for real* on the Mac; status streams back to the canvas live.
...
4. **Control model:** odin owns a desired-state spec + a **continuous reconciler/control loop** (not one-shot IaC apply). Nomad-shaped; Nomad itself is out (BUSL).
...
**Second load-bearing rule — one container-spawn authority:** odin's Runtime driver is the *only* code that runs a container. MiniStack keeps its per-service knowledge (which image/creds/config, the `CreateDBInstance`→Postgres trigger) but its container-spawn primitive is **rewired (overridden from an odin bootstrap) to call odin's Runtime driver and register the container in the World.** So an "RDS node" and a "user Postgres node" become the same Postgres container via the same run path — nothing double-spawns, the World is one coherent ledger, and AWS-service containers inherit supervision, memory budgeting, the overlay, live tiles, and AI monitoring for free.
...
- An in-process `httpx.AsyncClient(transport=ASGITransport(app=ministack_app))` for odin's own boto-shaped calls (zero network).
...
**Spawn rewire (mechanism confirmed by spec review, 2026-06-21):** MiniStack's container spawning is *scattered* across 8 service modules (no central helper). Each module has a private `_get_docker()` → module-global `_docker` (lambda_svc and opensearch use `_docker_client`) and calls `<client>.containers.run(...)` inline. We override **from an odin bootstrap, without editing service files**: eagerly `import ministack.services.rds as rds` then set `rds._docker = OdinDockerShim()`. The shim duck-types the `docker` client surface MiniStack uses (`.containers.run(image, environment, ports, name, labels, …) → obj` with `.id`/`.status`/`.reload()`/`.attrs`/`.remove()`/`.stop()`); inside `run()` odin's Runtime driver boots the real container (Lima/nerdctl) and the shim reports liveness, so MiniStack's control-plane bookkeeping (instance dict, status flip, endpoint) runs unchanged and no real Docker is touched. Patch *after* importing each module (lazy import resets the global). This is N per-service monkeypatches, one per real-container service we support — not one seam.
...
- **Do NOT run MiniStack's lifespan in-process.** Its startup/shutdown calls `_stop_docker_containers()` which force-removes any container labelled `ministack=rds|ecs|…` via a real `docker.from_env()` — it would reap odin's containers. `httpx.ASGITransport` does not run lifespan by default; keep it that way (the per-request path works without it). Use non-colliding container labels in the shim regardless.
- **Deps:** the `docker` SDK and `psycopg2` are NOT MiniStack base deps. The shim removes the need for a real docker daemon, but MiniStack's RDS readiness probe imports `psycopg2` to auth-probe the booted Postgres; add `psycopg2-binary` to odin's venv or readiness degrades to a flaky 1s TCP check.
- **Per-env (M6):** control-plane state is already account-scoped (a distinct 12-digit access key → isolated state, ARNs, uniqueness checks), so map env → synthetic account id. BUT real container names (`ministack-rds-{db_id}`) and the global host-port counter are NOT env-scoped — odin must namespace names/ports inside the runner shim.

A weekly GitHub Action keeps the fork synced (merge upstream + pytest + open a PR only on conflict); odin pins to a fork **commit/tag** the CI bumps and asserts the `_docker` monkeypatch targets exist at startup (fail fast on drift — MiniStack moves daily).

### 3.7 Networking and references

Nodes address each other *only* through resolved references, never hardcoded endpoints. A `${{node.VAR}}` on the canvas becomes a typed Ref edge in the Stack (auto-drawing the visual edge, preserving odin's `iam.ts` auto-detect UX). At reconcile time the **Fabric driver** (not the LLM) resolves: `resolve(ref) → Address` appropriate to the active fabric. localhost impl: each container gets a host port recorded as a World fact + a friendly `<node>.local` on the per-network Nebula overlay (reused from `VpcOverlay` + `NebulaManager`). A consumer never knows whether it's talking to localhost, `.local`, or (later) a Nebula overlay address.
...
#### 3.7.1 Self-hosted Nebula mesh fabric (the multi-Mac design)

**Why Nebula, not Tailscale** (user, 2026-06-22): Nebula runs inside your private network, *you* own the lighthouse, and a control plane / UI can be built on top of the mesh. Tailscale's SaaS coordination would limit that and conflicts with odin's own-your-control-loop ethos. Settled via a design panel + adversarial review; the panel caught and fixed six concrete bugs (recorded below).

- **One Nebula network == one environment.** The env is already odin's hard isolation unit (per-env reconciler, per-env MiniStack account, per-env `.odin/<env>/` lineage). `create_ca(env)` mints one CA per env, so a host in `prod` cannot present a valid cert to `staging`'s lighthouse — cross-env isolation is free at the PKI layer, mirroring the per-env AWS account boundary. (Rejected: one fleet-wide network, which re-introduces cross-env blast radius; per-deployment, which churns certs every apply.)
- **`resolve()` is unchanged** — `NebulaFabric` inherits `LocalhostFabric.resolve` verbatim (a verified drop-in: same `str` return, same two `Unresolved` raises). The overlay address rides into World facts through the *producer's* normal healthy-fact publication; the consumer just reads it. So a ref resolves to `10.42.1.7:6379` (mesh) exactly the way it resolves to `127.0.0.1:6379` (loopback) today.
- **Host overlay IP → World facts** (the only producer-side change, deferred): `HostFacts` gains `overlay_ip` (None today → producers publish `127.0.0.1`, byte-identical). When meshed, `_observe_container`/`_observe_rds` bind on `host.overlay_ip` instead of the loopback literal. **Verdict fix:** the host overlay IP is host-stable — resolve it *once per reconciler and cache it*, never call `ensure_host()` (which shells `docker info`/`nerdctl info`) per resource per tick.
- **Host-level addressing:** one cert per Mac; workloads are reached as `host-overlay-IP:host-port` (host_port is already the routable port). Fewest certs; per-workload network policy is explicitly out of scope.
- **Lighthouse:** exactly one per env, run by odin on the user's control-plane Mac (you own it). Members learn it via one out-of-band fact — the lighthouse's underlay LAN `ip:4242` — baked into each member's `static_host_map` by `generate_config`.
- **Cert lifecycle:** CA created lazily per env on first apply, destroyed on `/destroy?env=`; host certs signed at host-join (`sign_cert(hostname, overlay_ip_cidr, groups)`), revoked at leave/destroy. **Verdict fixes:** overlay IPs are **sticky by host_id** (`MeshNetwork.allocate_host` is idempotent; `load_overlay()` before allocating) so re-applies never invalidate already-published consumer env vars; the cert `-ip` is composed in CIDR form (`MeshNetwork.cert_ip`) since `nebula-cert` needs the mask; overlay state persists under `.odin/<env>/nebula/` (explicit `data_dir`, not a shared `~/.odin`); the recovered `sg_rules_to_firewall` derives ACLs, with a documented allow-all `DEFAULT_FIREWALL` until per-kind groups land.
- **Mesh UI hook** (the point of self-hosting): `mesh_state(root, env, world) -> MeshState` is a pure Pydantic projection over `overlay.json` + World, surfaced read-only at `GET /mesh?env=` (mirrors `/world?env=`); a future `mesh_delta` rides the existing per-env WebSocket (must carry `env`).
- **NOW (foundation, built + tested):** the recovered `NebulaManager` primitives (sync, injectable subprocess seam), `fabric/models.py`, `NebulaFabric` (resolve drop-in), `mesh_state` + `/mesh`, `HostFacts.overlay_ip`. **Deferred to M7:** cross-Mac **World replication** (the real blocker — World is one per-env doc on one Mac; resolve only works cross-host once World is gossiped/replicated via memberlist/raft), multi-Mac placement, real remote host join/leave + Lima cert injection (Colima has no VM to inject `/etc/nebula/*` into — an open question), and CRL-based revocation.
...
- **S0 — Embed MiniStack additively + prove the spawn-rewire in isolation.** Add the forked MiniStack dep + `psycopg2-binary` + `docker` SDK types; mount `ministack.app:app` at `/aws` in `create_app()` (additive — **do not** remove Moto/Tofu yet; set `MINISTACK_HOST`/`MINISTACK_ACCOUNT_ID` before import; never run its lifespan). Build a minimal `OdinDockerShim` + a thin runner that boots a real Postgres (reuse `ContainerManager`/`VmManager`). Monkeypatch `rds._docker = shim` from a bootstrap. Acceptance (a standalone integration test, app still boots normally): an in-process boto RDS `CreateDBInstance` → real Postgres up via odin's runner → `DescribeDBInstances` returns an `available` endpoint reachable at `127.0.0.1:host_port`.
