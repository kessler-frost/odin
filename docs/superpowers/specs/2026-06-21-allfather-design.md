...
**Why Nebula, not Tailscale** (user, 2026-06-22): Nebula runs inside your private network, *you* own the lighthouse, and a control plane / UI can be built on top of the mesh. Tailscale's SaaS coordination would limit that and conflicts with allfather's own-your-control-loop ethos. Settled via a design panel + adversarial review; the panel caught and fixed six concrete bugs (recorded below).

- **One Nebula network == one environment.** The env is already allfather's hard isolation unit (per-env reconciler, per-env MiniStack account, per-env `.odin/<env>/` lineage). `create_ca(env)` mints one CA per env, so a host in `prod` cannot present a valid cert to `staging`'s lighthouse — cross-env isolation is free at the PKI layer, mirroring the per-env AWS account boundary. (Rejected: one fleet-wide network, which re-introduces cross-env blast radius; per-deployment, which churns certs every apply.)
...
- **Lighthouse:** exactly one per env, run by allfather on the user's control-plane Mac (you own it). Members learn it via one out-of-band fact — the lighthouse's underlay LAN `ip:4242` — baked into each member's `static_host_map` by `generate_config`.
