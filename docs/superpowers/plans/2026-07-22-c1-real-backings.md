...
**Step 0a (bug found by Task 3 — fix first, gates the sns e2e):** canvas edges carry ReactFlow node IDs (`e.source`/`e.target`, e.g. `"n1"`) but Stack resources are keyed by node LABEL (`translate._node_id` uses `data.label`), so the reconciler's sns-subscription matching (`e.src == action.id`) silently misses whenever id ≠ label. Fix in `src/odin/spec/translate.py`: `canvas_to_stack` builds an id→label map from the nodes list and `_edge` translates `source`/`target` through it (fall back to the raw value for edges naming labels directly — Stack-level tests do this). Unit tests in `tests/spec/test_translate.py`: an edge `n1→n2` between nodes labeled `alerts`/`jobs` yields `Edge(src="alerts", dst="jobs")`; an edge already using labels is unchanged.

**Step 0b (do first — a container-churn suite needs it):** `ColimaRuntime.stop` (src/odin/runtime/colima.py:157-158) is `docker rm -f` WITHOUT `-v`, so every removed container leaks its anonymous volumes (postgres creates one per boot — the host accumulated 2.4GB of them before this plan). Change to `self._cli("rm", "-f", "-v", name, check=False)`, update any unit test asserting stop's CLI args, and add to this task's final gate: `docker volume ls -qf dangling=true` is empty after the full integration sweep.
...
- Modify: `ui/src/lib/iam.ts` — GENERALIZE, do not gut (user directive: edges = IAM permissions with full AWS verbs = the core UX; the C1.5 gateway enforces them for real within this release — see the spec §C1). `computeTypes` (`{ec2, lambda}`) becomes the workload set `{service, app, batch, llm}`; `iamActionsForTarget` keeps s3/dynamodb + the catalog spread (sqs/sns/rds actions come from `catalog.ts` `iamActions`); `catalogIamPairs`/`edgeTypesForPair` pair workloads↔resources instead of ec2/lambda↔resources. Permission edges must still be drawable between a workload node and any s3/sqs/sns/dynamodb/rds node, with the existing AWS-verb picker UX fully working. No "not enforced" copy — enforcement ships in the same release.
