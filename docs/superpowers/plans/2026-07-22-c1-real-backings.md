...
- Modify: `ui/src/lib/iam.ts` — GENERALIZE, do not gut (standing decision: edges-with-permissions are the access-graph model, later compiled to Nebula firewall rules at M7 — see the spec §C1). `computeTypes` (`{ec2, lambda}`) becomes the workload set `{service, app, batch, llm}`; `iamActionsForTarget` keeps s3/dynamodb + the catalog spread (sqs/sns/rds actions come from `catalog.ts` `iamActions`); `catalogIamPairs`/`edgeTypesForPair` pair workloads↔resources instead of ec2/lambda↔resources. Permission edges must still be drawable between a workload node and any s3/sqs/sns/dynamodb/rds node, and any existing "permissions" picker UX keeps working. Where the UI surfaces permissions, the copy must say "modeled, not enforced" (tooltip or sublabel — smallest honest change).
