...
- [ ] **Step 1 — Pin upstream MiniStack from PyPI (no fork yet).** The skeleton's spawn rewire is an *external* monkeypatch (no MiniStack source edits), so a fork is unnecessary until later milestones modify MiniStack files. `uv add "ministack==<pinned>"` + `uv add psycopg2-binary`. (MiniStack is MIT; psycopg2-binary is LGPL-3 with a linking exception → acceptable.) The `docker` python package is NOT needed (the monkeypatch makes `_get_docker()` return our shim before it ever `import docker`). Defer the fork to M2 (when we edit MiniStack source) → then `kessler-frost/ministack` + the auto-sync CI.
