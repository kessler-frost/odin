...
- sns/iam/elbv2:  query-XML, wrapped in ``<ErrorResponse><Error>...`` --
                  all three are botocore's "query" protocol (verified against
                  each service's own model), whose error parser looks one
                  level deeper than S3's REST-XML. IAM and elbv2
                  (`elasticloadbalancing`, task W2.5) share SNS's exact
                  envelope shape, so all three route through `_sns_xml`.
...
# The services whose error envelope is botocore's query-XML
# ``<ErrorResponse><Error>`` shape (module docstring).
_QUERY_XML_SERVICES = ("sns", "iam", "elasticloadbalancing")
