...
- sns/iam/rds:    query-XML, wrapped in ``<ErrorResponse><Error>...`` --
                  all three are botocore's "query" protocol (verified against
                  each service's own model), whose error parser looks one
                  level deeper than S3's REST-XML. IAM and RDS (task W2.7)
                  share SNS's exact envelope shape, so all three route
                  through `_sns_xml`.
