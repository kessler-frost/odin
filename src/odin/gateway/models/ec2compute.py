...
def _sg_firewall(stores: SynthStores, env: str, group_id: str) -> FirewallRules | None:
    """One security group's ALREADY-compiled Nebula firewall
    (`ec2net.py::_compiled_firewall`, recomputed on every SG mutation and
    stored on the SG record itself -- read here, not recomputed, matching
    `fabric.nebula.mesh_state`'s own "read the sidecar, don't reach into
    ec2net" boundary)."""
    sg = stores.ec2net.get(env, f"sg:{group_id}")
...
def _instance_firewall(stores: SynthStores, env: str, vpc: dict, security_group_ids: list[str]) -> FirewallRules | None:
    ...
    assigned = [f for gid in security_group_ids if (f := _sg_firewall(stores, env, gid)) is not None]
    if assigned:
        return union_firewalls(assigned)
    return _sg_firewall(stores, env, vpc["default_sg_id"])
