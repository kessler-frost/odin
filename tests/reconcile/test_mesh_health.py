...
class FakeRuntime:
    ...
    async def container_id(self, name):
        self.calls.append(("container_id", name))
        return self._target_id

    async def network_mode(self, name):
        self.calls.append(("network_mode", name))
        return self._netns

    async def exec_sh(self, name, script):
...
async def _gate(tmp_path, entry, *, runtime=None, lighthouse=None, now=0.0, overlay_ip=OVERLAY_IP, **kwargs):
    return await mesh_health.gate(
...
async def test_no_mesh_fact_means_no_check_at_all(tmp_path):
    ...
    assert await _gate(tmp_path, entry, runtime=runtime) == entry
    assert runtime.calls == []


async def test_no_overlay_ip_means_no_check(tmp_path):
    runtime = FakeRuntime()
    entry = ("rds", "healthy", dict(MESH_FACTS), None)
    assert await _gate(_meshed_env(tmp_path), entry, runtime=runtime, overlay_ip=None) == entry
    assert runtime.calls == []


async def test_an_env_without_a_nebula_ca_is_never_faulted(tmp_path):
    ...
    entry = ("rds", "healthy", dict(MESH_FACTS), None)
    assert (await _gate(tmp_path, entry))[1] == "healthy"


# --- a live overlay path is left alone -----------------------------------------


async def test_a_live_mesh_endpoint_keeps_the_fact_and_the_phase(tmp_path):
    runtime = FakeRuntime(probe_ok=True)
    entry = ("rds", "healthy", dict(MESH_FACTS), None)
    assert await _gate(_meshed_env(tmp_path), entry, runtime=runtime) == entry
...
async def test_a_dead_lighthouse_withholds_the_mesh_fact_and_ends_healthy(tmp_path):
    ...
    kind, phase, facts, verdict = await _gate(
...
async def test_a_missing_nebula_binary_is_named_with_the_command_that_fixes_it(tmp_path):
...
async def test_every_failure_verdict_names_a_reachable_fix(tmp_path):
    ...
    for name, kwargs in faults.items():
        mesh_health.reset_cache()
        _, phase, _, verdict = await _gate(root, ("rds", "healthy", dict(MESH_FACTS), None), **kwargs)
...
async def test_a_check_that_explodes_does_not_tell_the_user_to_re_apply(tmp_path):
...
async def test_a_sidecar_in_a_replaced_namespace_is_reported(tmp_path):
    ...
    _, phase, facts, verdict = await _gate(
...
async def test_a_stopped_sidecar_is_reported(tmp_path):
    _, phase, _, verdict = await _gate(
        _meshed_env(tmp_path), ("rds", "healthy", dict(MESH_FACTS), None),
...
async def test_an_unanswered_overlay_address_carries_the_probes_real_words(tmp_path):
    _, phase, _, verdict = await _gate(
        _meshed_env(tmp_path), ("rds", "healthy", dict(MESH_FACTS), None),
...
async def test_an_already_crashed_resource_keeps_its_own_verdict(tmp_path):
...
async def test_a_passing_check_is_cached_for_the_sweep_window(tmp_path):
    root = _meshed_env(tmp_path)
    runtime = FakeRuntime(probe_ok=True)
    entry = ("rds", "healthy", dict(MESH_FACTS), None)
    for tick in range(30):  # 30 reconciler ticks at 1s
        await _gate(root, entry, runtime=runtime, now=float(tick))
    assert len([c for c in runtime.calls if c[0] == "exec_sh"]) == 1
    await _gate(root, entry, runtime=runtime, now=31.0)  # past ODIN_MESH_SWEEP_SECONDS
    assert len([c for c in runtime.calls if c[0] == "exec_sh"]) == 2


async def test_a_failing_check_is_re_checked_sooner_so_a_recovery_is_not_stale(tmp_path):
    root = _meshed_env(tmp_path)
    runtime = FakeRuntime(probe_ok=False)
    entry = ("rds", "healthy", dict(MESH_FACTS), None)
    assert (await _gate(root, entry, runtime=runtime, now=0.0))[1] == "crashed"
    assert (await _gate(root, entry, runtime=runtime, now=3.0))[1] == "crashed"
    assert len([c for c in runtime.calls if c[0] == "exec_sh"]) == 1
    runtime.probe_ok = True  # the Apply re-joined it
    assert (await _gate(root, entry, runtime=runtime, now=6.0))[1] == "healthy"


async def test_the_cadence_is_configurable(tmp_path, monkeypatch):
    monkeypatch.setenv("ODIN_MESH_SWEEP_SECONDS", "120")
    root = _meshed_env(tmp_path)
    runtime = FakeRuntime(probe_ok=True)
    entry = ("rds", "healthy", dict(MESH_FACTS), None)
    await _gate(root, entry, runtime=runtime, now=0.0)
    await _gate(root, entry, runtime=runtime, now=100.0)
    assert len([c for c in runtime.calls if c[0] == "exec_sh"]) == 1


async def test_a_check_that_explodes_is_a_verdict_not_a_crashed_tick(tmp_path):
    class Exploding(FakeRuntime):
        async def status(self, name):
            raise RuntimeError("docker daemon is not responding")
...
async def test_a_vm_member_is_checked_against_the_lighthouse_only(tmp_path):
    ...
    mesh_health.reset_cache()
    _, phase, facts, verdict = await mesh_health.gate(
        entry, root=_meshed_env(tmp_path), env=ENV, member="i-abc", overlay_ip="10.42.1.2",
...
async def test_mesh_ready_reports_the_probes_own_failure_text():
    class Silent:
        async def exec_sh(self, name, script):
            return ""

    result = await mesh_ready_sync(Silent(), SIDECAR, OVERLAY_IP, PORT)
