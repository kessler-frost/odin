...
`db-sg`. Everything downstream is real: tofu creates the VPC/subnets/SGs
through the gateway, two real Lima VMs boot and join the env's Nebula mesh
with their ASSIGNED SG's firewall + their sg ids as cert groups (piece 1), and
the reconciler's Postgres container joins the SAME mesh behind `db-sg`'s
...
def _db_facts(client: TestClient, timeout: float = 180.0) -> dict:
    ...
    overlay address (the reconciler's observe pass joins the mesh, so the
    mesh fact appears a tick or two after apply returns)."""
    deadline = time.monotonic() + timeout
    last: dict = {}
    while time.monotonic() < deadline:
...
def test_a_drawn_sg_lets_the_web_vm_reach_postgres_and_refuses_the_other(
    ...
    with TestClient(app) as client:
        ...
        # rds is deliberately NOT a tofu resource (the reconciler owns it) --
        # it's the ONLY thing this canvas can't hand to tofu.
        ...
        # The compiled firewall really is `db-sg`'s, resolved BY NAME through
        # the gateway's own sidecar file -- and it names web-sg's group id, so
        # nebula matches it against a peer's certificate groups.
