...
@respx.mock
async def test_tf_status_json(runner):
    respx.get(f"{BASE}/tf/status", params={"env": "default"}).mock(
        return_value=httpx.Response(200, json=STATUS_IDLE)
    )
    result = await runner.invoke(app, ["tf", "status", "-o", "json"])
    assert result.exit_code == 0
    assert json.loads(result.stdout) == STATUS_IDLE


@respx.mock
async def test_tf_status_text_idle(runner):
    respx.get(f"{BASE}/tf/status", params={"env": "default"}).mock(
        return_value=httpx.Response(200, json=STATUS_IDLE)
    )
    result = await runner.invoke(app, ["tf", "status"])
    assert result.exit_code == 0
...
@respx.mock
async def test_tf_status_text_with_failed_last_run(runner):
    respx.get(f"{BASE}/tf/status", params={"env": "prod"}).mock(
        return_value=httpx.Response(200, json=STATUS_LAST_FAILED)
    )
    result = await runner.invoke(app, ["tf", "status", "--env", "prod"])
    assert result.exit_code == 0
...
@respx.mock
async def test_tf_status_server_down(runner):
...
@respx.mock
async def test_tf_plan_with_changes_exits_two(runner):
    respx.post(f"{BASE}/tf/plan", params={"env": "default"}).mock(
...
@respx.mock
async def test_tf_plan_error_exits_one(runner):
...
@respx.mock
async def test_tf_plan_json_mode_keeps_the_exit_code(runner):
    respx.post(f"{BASE}/tf/plan").mock(return_value=httpx.Response(200, json=PLAN_CHANGES))
    result = await runner.invoke(app, ["tf", "plan", "-o", "json"])
    assert result.exit_code == 2
    assert json.loads(result.stdout) == PLAN_CHANGES


@respx.mock
async def test_tf_plan_names_nodes_the_plan_could_not_cover(runner):
...
@respx.mock
async def test_tf_plan_names_a_kind_odin_does_not_model_at_all(runner):
...
@respx.mock
async def test_tf_plan_json_publishes_the_same_gate_field_as_apply(runner):
    dropped = {**PLAN_NO_CHANGES, "skipped": ["kinesis"], "not_covered": ["kinesis"]}
    respx.post(f"{BASE}/tf/plan").mock(return_value=httpx.Response(200, json=dropped))
    body = json.loads(await runner.invoke(app, ["tf", "plan", "-o", "json"]).stdout)
    assert body["skipped"] == ["kinesis"]
    assert body["not_covered"] == ["kinesis"]


@respx.mock
async def test_tf_plan_prints_the_servers_note_when_the_two_halves_describe_different_things(runner):
...
@respx.mock
async def test_tf_plans_drift_note_is_on_stderr_like_its_sibling(runner):
...
@respx.mock
async def test_tf_plan_json_mode_still_emits_only_json_on_stdout(runner):
...
@respx.mock
async def test_tf_plan_json_carries_canvas_drift_for_a_ci_gate(runner):
...
@respx.mock
async def test_tf_plan_never_calls_the_canvas_route(runner):
    ...
    assert await runner.invoke(app, ["tf", "plan"]).exit_code == 0
    assert not canvas.called


@respx.mock
async def test_tf_plan_busy_409_exits_one(runner):
...
@respx.mock
async def test_tf_plan_server_down_exits_three_not_two(runner):
...
@respx.mock
async def test_tf_destroy_ok(runner):
...
@respx.mock
async def test_tf_destroy_failed_prints_tail_and_exits_nonzero(runner):
...
@respx.mock
async def test_tf_destroy_failed_json_mode(runner):
    body = {"status": "failed", "env": "default", "exit_code": 1, "tail": ["Error: kaboom"]}
    respx.post(f"{BASE}/tf/destroy").mock(return_value=httpx.Response(500, json=body))
    result = await runner.invoke(app, ["tf", "destroy", "-o", "json"])
    assert result.exit_code == 1
    assert json.loads(result.stdout) == body


@respx.mock
async def test_tf_destroy_tofu_not_installed_409(runner):
...
@respx.mock
async def test_tf_destroy_busy_409(runner):
    respx.post(f"{BASE}/tf/destroy").mock(
        return_value=httpx.Response(409, json={"error": "a tofu run is already in progress for env 'default'"})
    )
    result = await runner.invoke(app, ["tf", "destroy"])
...
@respx.mock
async def test_tf_destroy_server_down(runner):
