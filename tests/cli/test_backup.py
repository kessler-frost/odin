...
async def test_export_writes_manifest_and_every_expected_member(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    _seed(tmp_path / ".odin")
    result = await runner.invoke(app, ["export", "--env", ENV])
...
async def test_export_excludes_the_terraform_provider_cache(runner, tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    _seed(tmp_path / ".odin")
    assert await runner.invoke(app, ["export", "--env", ENV]).exit_code == 0
...
async def test_export_honours_the_out_path_and_json_output(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    _seed(tmp_path / ".odin")
    dest = tmp_path / "nested" / "snap.tar.gz"
    result = await runner.invoke(app, ["export", "--env", ENV, "-o", str(dest), "--output", "json"])
    assert result.exit_code == 0, result.output
...
async def test_export_warns_that_the_archive_holds_credentials(runner, tmp_path, monkeypatch):
    ...
    assert json.loads(await runner.invoke(app, ["export", "--env", ENV, "--output", "json"]).stdout)


async def test_export_of_an_unknown_env_fails_with_the_known_ones(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    _seed(tmp_path / ".odin")
    result = await runner.invoke(app, ["export", "--env", "nope"])
...
async def test_round_trip_restores_every_file_byte_for_byte_and_keeps_0600(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    root = tmp_path / ".odin"
    env_dir = _seed(root)
    before = {
    ...
    result = await runner.invoke(app, ["import", f"odin-{ENV}-export.tar.gz"])
    assert result.exit_code == 0, result.output
...
async def test_import_leaves_the_canvas_alone_unless_asked(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    root = tmp_path / ".odin"
    _seed(root)
    assert await runner.invoke(app, ["export", "--env", ENV]).exit_code == 0
    ...
    plain = await runner.invoke(app, ["import", f"odin-{ENV}-export.tar.gz", "--force"])
    assert plain.exit_code == 0, plain.output
    assert "Canvas left alone" in plain.stdout
    assert "mine" in (root / CANVAS_NAME).read_text()  # NOT clobbered

    withc = await runner.invoke(app, ["import", f"odin-{ENV}-export.tar.gz", "--force", "--with-canvas"])
    assert withc.exit_code == 0, withc.output
    assert "Canvas restored" in withc.stdout
    assert "n1" in (root / CANVAS_NAME).read_text()


async def test_import_env_override_rewrites_the_env_dir_name(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    root = tmp_path / ".odin"
    _seed(root)
    assert await runner.invoke(app, ["export", "--env", ENV]).exit_code == 0
    result = await runner.invoke(app, ["import", f"odin-{ENV}-export.tar.gz", "--env", "restored"])
    assert result.exit_code == 0, result.output
...
async def test_import_json_output_reports_both_env_names(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    _seed(tmp_path / ".odin")
    assert await runner.invoke(app, ["export", "--env", ENV]).exit_code == 0
...
async def test_import_refuses_an_existing_env_without_force(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    root = tmp_path / ".odin"
    _seed(root)
    assert await runner.invoke(app, ["export", "--env", ENV]).exit_code == 0
    ...
    refused = await runner.invoke(app, ["import", f"odin-{ENV}-export.tar.gz"])
    ...
    forced = await runner.invoke(app, ["import", f"odin-{ENV}-export.tar.gz", "--force"])
    assert forced.exit_code == 0, forced.output
    assert "live" not in (root / ENV / "world.json").read_text()


async def test_force_import_replaces_rather_than_merges(runner, tmp_path, monkeypatch):
    ...
    assert await runner.invoke(app, ["import", f"odin-{ENV}-export.tar.gz", "--force"]).exit_code == 0
    assert not (root / ENV / "stacks" / "stale.json").exists()


async def test_import_refuses_a_live_server(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
...
async def test_import_can_be_forced_past_the_live_server_check(runner, tmp_path, monkeypatch):
    ...
    result = await runner.invoke(app, ["import", f"odin-{ENV}-export.tar.gz", "--ignore-live-server"])
    assert result.exit_code == 0, result.output
    assert (root / ENV / "HEAD").read_text() == "abc123"


async def test_import_proceeds_past_a_stale_pidfile(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    root = tmp_path / ".odin"
    _seed(root)
    assert await runner.invoke(app, ["export", "--env", ENV]).exit_code == 0
...
async def test_import_of_a_missing_archive_fails_cleanly(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    result = await runner.invoke(app, ["import", "nowhere.tar.gz"])
    assert result.exit_code == 1
...
async def test_unknown_format_exits_2(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    ...
    result = await runner.invoke(app, ["import", str(archive)])
...
async def test_a_tarball_that_is_not_an_odin_export_exits_2(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
...
async def test_a_truncated_archive_fails_with_a_clear_message(runner, tmp_path, monkeypatch):
    ...
    result = await runner.invoke(app, ["import", str(truncated)])
    assert result.exit_code == 2
...
async def test_a_file_that_is_not_gzip_at_all_fails_with_a_clear_message(runner, tmp_path, monkeypatch):
    ...
    result = await runner.invoke(app, ["import", str(plain)])
    assert result.exit_code == 2
...
async def test_an_empty_file_fails_with_a_clear_message(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    empty = tmp_path / "empty.tar.gz"
    empty.write_bytes(b"")
    result = await runner.invoke(app, ["import", str(empty)])
    assert result.exit_code == 2
    assert "not a readable .tar.gz" in result.stderr and "Traceback" not in result.stderr


async def test_a_directory_given_where_an_archive_belongs_fails_cleanly(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "adir").mkdir()
    result = await runner.invoke(app, ["import", str(tmp_path / "adir")])
    assert result.exit_code == 1
    assert "no such archive" in result.stderr and "Traceback" not in result.stderr


async def test_a_corrupt_manifest_exits_2(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
...
async def test_import_rejects_traversal_members(runner, tmp_path, monkeypatch, evil):
    monkeypatch.chdir(tmp_path)
    archive = _write_archive(
