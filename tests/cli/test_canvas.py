...
@respx.mock
async def test_canvas_get_json(runner):
    respx.get(f"{BASE}/canvas").mock(return_value=httpx.Response(200, json=GRAPH))
    result = await runner.invoke(app, ["canvas", "get", "-o", "json"])
    assert result.exit_code == 0
    assert json.loads(result.stdout) == GRAPH


@respx.mock
async def test_canvas_get_defaults_to_json_so_it_can_be_piped(runner):
    """FRICTION-1: the README's `odin canvas get | jq …` died on the first pipe
    because the default was a one-line human summary."""
    respx.get(f"{BASE}/canvas").mock(return_value=httpx.Response(200, json=GRAPH))
    result = await runner.invoke(app, ["canvas", "get"])
    assert result.exit_code == 0
    assert json.loads(result.stdout) == GRAPH


@respx.mock
async def test_canvas_get_text(runner):
    respx.get(f"{BASE}/canvas").mock(return_value=httpx.Response(200, json=GRAPH))
    result = await runner.invoke(app, ["canvas", "get", "-o", "text"])
    assert result.exit_code == 0
...
@respx.mock
async def test_canvas_get_honors_url_option_and_envvar(runner):
    respx.get("http://odin.local:9999/canvas").mock(return_value=httpx.Response(200, json=GRAPH))
    assert await runner.invoke(app, ["canvas", "get", "--url", "http://odin.local:9999"]).exit_code == 0
    assert await runner.invoke(app, ["canvas", "get"], env={"ODIN_URL": "http://odin.local:9999"}).exit_code == 0


@respx.mock
async def test_canvas_get_server_down(runner):
...
@respx.mock
async def test_canvas_set_from_file(runner, tmp_path):
    ...
    result = await runner.invoke(app, ["canvas", "set", str(canvas_file)])
    assert result.exit_code == 0
...
@respx.mock
async def test_canvas_set_from_stdin_json_mode(runner):
    respx.post(f"{BASE}/canvas").mock(return_value=httpx.Response(200, json={"status": "saved"}))
    result = await runner.invoke(app, ["canvas", "set", "-", "-o", "json"], input=json.dumps(GRAPH))
    assert result.exit_code == 0
    assert json.loads(result.stdout) == {"status": "saved"}


async def test_canvas_set_rejects_malformed_json(runner, tmp_path):
    bad = tmp_path / "bad.json"
    bad.write_text("{nope")
    result = await runner.invoke(app, ["canvas", "set", str(bad)])
    assert result.exit_code == 1
    assert "not valid JSON" in result.stderr


@respx.mock
async def test_canvas_set_refuses_a_structurally_broken_canvas_without_posting_it(runner, tmp_path):
...
@respx.mock
async def test_canvas_set_still_accepts_a_kind_odin_cannot_build(runner):
    ...
    result = await runner.invoke(app, ["canvas", "set", "-"], input=json.dumps(graph))
    assert result.exit_code == 0
    assert json.loads(route.calls.last.request.content) == graph


@respx.mock
async def test_canvas_set_places_a_node_with_no_position(runner):
    ...
    result = await runner.invoke(app, ["canvas", "set", "-"], input=json.dumps(NO_POSITION))
    assert result.exit_code == 0
...
@respx.mock
async def test_canvas_set_places_many_nodes_on_a_20px_grid(runner):
    route = respx.post(f"{BASE}/canvas").mock(
        return_value=httpx.Response(200, json={"status": "saved"})
    )
    graph = {"nodes": [{"id": f"n{i}", "type": "s3"} for i in range(7)], "edges": []}
    assert await runner.invoke(app, ["canvas", "set", "-"], input=json.dumps(graph)).exit_code == 0
...
@respx.mock
async def test_canvas_set_leaves_real_positions_alone(runner):
    route = respx.post(f"{BASE}/canvas").mock(
        return_value=httpx.Response(200, json={"status": "saved"})
    )
    result = await runner.invoke(app, ["canvas", "set", "-"], input=json.dumps(GRAPH))
...
@respx.mock
async def test_canvas_set_server_down(runner):
    respx.post(f"{BASE}/canvas").mock(side_effect=httpx.ConnectTimeout("slow"))
    result = await runner.invoke(app, ["canvas", "set", "-"], input="{}")
    assert result.exit_code == 2
