...
@respx.mock
async def test_translate_default_posts_the_saved_canvas(runner):
    ...
    result = await runner.invoke(app, ["translate"])
...
@respx.mock
async def test_translate_with_file_posts_that_canvas(runner, tmp_path):
    ...
    result = await runner.invoke(app, ["translate", "--env", "prod", "--file", str(canvas_file)])
    assert result.exit_code == 0
    assert json.loads(route.calls.last.request.content) == GRAPH


@respx.mock
async def test_translate_notes_an_empty_saved_canvas_on_stderr(runner):
...
@respx.mock
async def test_translate_json_mode_prints_full_result(runner):
    ...
    result = await runner.invoke(app, ["translate", "-o", "json"])
    assert result.exit_code == 0
...
@respx.mock
async def test_translate_server_down(runner):
...
@respx.mock
async def test_import_tf_hcl_mode_stdout_is_canvas_shaped(runner, tmp_path):
    tf_file = tmp_path / "main.tf"
    tf_file.write_text(MAIN_TF)
    route = respx.post(f"{BASE}/import-tf").mock(return_value=httpx.Response(200, json=IMPORTED))
    result = await runner.invoke(app, ["import-tf", str(tf_file)])
    assert result.exit_code == 0
...
@respx.mock
async def test_import_tf_json_mode_same_stdout(runner, tmp_path):
    tf_file = tmp_path / "main.tf"
    tf_file.write_text(MAIN_TF)
    respx.post(f"{BASE}/import-tf").mock(return_value=httpx.Response(200, json=IMPORTED))
    result = await runner.invoke(app, ["import-tf", str(tf_file), "-o", "json"])
    assert result.exit_code == 0
    assert json.loads(result.stdout) == {"nodes": IMPORTED["nodes"], "edges": []}


@respx.mock
async def test_import_tf_live_mode(runner):
...
async def test_import_tf_live_malformed_spec(runner):
    result = await runner.invoke(app, ["import-tf", "--live", "s3uploads"])
    assert result.exit_code == 2
    assert "type=id" in result.stderr


async def test_import_tf_requires_file_or_live(runner):
    result = await runner.invoke(app, ["import-tf"])
    assert result.exit_code == 2
...
@respx.mock
async def test_import_tf_accepts_a_directory_and_sends_every_tf_as_one_canvas(runner, tmp_path):
    ...
    result = await runner.invoke(app, ["import-tf", str(project)])
...
async def test_import_tf_of_a_directory_with_no_tf_files_says_so(runner, tmp_path):
    empty = tmp_path / "empty"
    empty.mkdir()
    result = await runner.invoke(app, ["import-tf", str(empty)])
    assert result.exit_code == 2
    assert "no *.tf files in" in result.stderr and "Traceback" not in result.stderr


async def test_import_tf_of_a_missing_path_says_so_instead_of_tracebacking(runner, tmp_path):
    result = await runner.invoke(app, ["import-tf", str(tmp_path / "nope.tf")])
    assert result.exit_code == 2
    assert "no such file or directory" in result.stderr and "Traceback" not in result.stderr


@respx.mock
async def test_import_tf_malformed_hcl_exits_nonzero(runner, tmp_path):
...
@respx.mock
async def test_import_tf_only_unsupported_stays_exit_zero(runner, tmp_path):
...
@respx.mock
async def test_import_tf_server_down(runner, tmp_path):
    tf_file = tmp_path / "main.tf"
    tf_file.write_text(MAIN_TF)
