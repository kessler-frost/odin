...
async def test_keys_issue_text_mode(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    result = await runner.invoke(app, ["keys", "issue", "web", "--env", "prod"])
...
async def test_keys_issue_json_mode_matches_keystore(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    result = await runner.invoke(app, ["keys", "issue", "web", "-o", "json"])
    assert result.exit_code == 0
...
async def test_keys_issue_is_stable_per_env_node(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    first = await runner.invoke(app, ["keys", "issue", "web", "-o", "json"])
    second = await runner.invoke(app, ["keys", "issue", "web", "-o", "json"])
    assert json.loads(first.stdout) == json.loads(second.stdout)
    other_node = await runner.invoke(app, ["keys", "issue", "worker", "-o", "json"])
    assert json.loads(other_node.stdout) != json.loads(first.stdout)


async def test_keys_issue_warns_when_node_is_not_in_the_applied_stack(runner, tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    result = await runner.invoke(app, ["keys", "issue", "stranger", "--env", "prod"])
...
async def test_keys_issue_does_not_warn_for_a_node_in_the_applied_stack(runner, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
...
async def test_keys_issue_help_documents_the_escape_hatch(runner):
    result = await runner.invoke(app, ["keys", "issue", "--help"])
    assert result.exit_code == 0
