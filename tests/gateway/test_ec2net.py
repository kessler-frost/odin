...
async def _create_vpc(stores, sink, ec2, cidr="10.0.0.0/16", **kwargs) -> str:
    req = sink.call(lambda: ec2.create_vpc(CidrBlock=cidr, **kwargs))
    parsed = _parse("CreateVpc", await _answer(stores, req))
    return parsed["Vpc"]["VpcId"]


async def _create_sg(stores, sink, ec2, vpc_id: str, name="web") -> str:
...
async def _describe_sg(stores, sink, ec2, group_id: str) -> dict:
    req = sink.call(lambda: ec2.describe_security_groups(GroupIds=[group_id]))
    return _parse("DescribeSecurityGroups", await _answer(stores, req))["SecurityGroups"][0]


# --- VPC ----------------------------------------------------------------------


async def test_create_vpc_mints_aws_shaped_id_and_echoes_request(sink, ec2, stores):
...
async def test_describe_vpcs_all_and_by_id(sink, ec2, stores):
    ...
    parsed = _parse("DescribeVpcs", await _answer(stores, all_req))
    assert {v["VpcId"] for v in parsed["Vpcs"]} == {vpc_a, vpc_b}

    one_req = sink.call(lambda: ec2.describe_vpcs(VpcIds=[vpc_b]))
    parsed = _parse("DescribeVpcs", await _answer(stores, one_req))
    assert [v["VpcId"] for v in parsed["Vpcs"]] == [vpc_b]
    assert parsed["Vpcs"][0]["CidrBlock"] == "10.1.0.0/16"


async def test_describe_vpcs_unknown_id_is_the_real_not_found_envelope(sink, ec2, stores):
...
async def test_describe_vpc_attribute_defaults(sink, ec2, stores):
    ...
    parsed = _parse("DescribeVpcAttribute", await _answer(stores, dns_req))
    ...
    parsed = _parse("DescribeVpcAttribute", await _answer(stores, host_req))
...
async def test_create_vpc_auto_creates_default_nacl(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    req = sink.call(lambda: ec2.describe_network_acls(
        Filters=[{"Name": "vpc-id", "Values": [vpc_id]}, {"Name": "default", "Values": ["true"]}],
    ))
    parsed = _parse("DescribeNetworkAcls", await _answer(stores, req))
...
async def test_create_vpc_auto_creates_main_route_table(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    req = sink.call(lambda: ec2.describe_route_tables(
        Filters=[{"Name": "vpc-id", "Values": [vpc_id]}, {"Name": "association.main", "Values": ["true"]}],
    ))
    parsed = _parse("DescribeRouteTables", await _answer(stores, req))
...
async def test_create_vpc_auto_creates_default_security_group(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    req = sink.call(lambda: ec2.describe_security_groups(
        Filters=[{"Name": "vpc-id", "Values": [vpc_id]}, {"Name": "group-name", "Values": ["default"]}],
    ))
    parsed = _parse("DescribeSecurityGroups", await _answer(stores, req))
...
async def test_create_subnet_describe_delete_round_trip(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    create_req = sink.call(lambda: ec2.create_subnet(
        VpcId=vpc_id, CidrBlock="10.0.1.0/24", AvailabilityZone="us-east-1a",
    ))
    created = _parse("CreateSubnet", await _answer(stores, create_req))["Subnet"]
    ...
    parsed = _parse("DescribeSubnets", await _answer(stores, by_vpc))
...
async def test_create_security_group_seeds_default_allow_all_egress(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    group_id = await _create_sg(stores, sink, ec2, vpc_id)
    assert group_id.startswith("sg-") and len(group_id) == len("sg-") + 17

    sg = await _describe_sg(stores, sink, ec2, group_id)
...
async def test_provider_can_revoke_the_seeded_default_egress(sink, ec2, stores):
    ...
    assert (await _answer(stores, revoke_req)).status_code == 200
    assert (await _describe_sg(stores, sink, ec2, group_id))["IpPermissionsEgress"] == []


async def test_authorize_ingress_is_idempotent_by_content(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    group_id = await _create_sg(stores, sink, ec2, vpc_id)
    permissions = [
    ...
    parsed = _parse("AuthorizeSecurityGroupIngress", await _answer(stores, first))
    ...
    parsed = _parse("AuthorizeSecurityGroupIngress", await _answer(stores, again))
    ...
    rules = _parse("DescribeSecurityGroupRules", await _answer(stores, rules_req))["SecurityGroupRules"]
...
async def test_revoke_ingress_matches_by_content_hash(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    group_id = await _create_sg(stores, sink, ec2, vpc_id)
    ...
    await _answer(stores, auth_req)

    revoke_req = sink.call(lambda: ec2.revoke_security_group_ingress(GroupId=group_id, IpPermissions=perm))
    assert (await _answer(stores, revoke_req)).status_code == 200
    assert (await _describe_sg(stores, sink, ec2, group_id))["IpPermissions"] == []


async def test_describe_security_groups_aggregates_ranges_per_permission(sink, ec2, stores):
    ...
    await _answer(stores, auth_req)
    (ingress,) = (await _describe_sg(stores, sink, ec2, group_id))["IpPermissions"]
    assert ingress["FromPort"] == 443 and ingress["ToPort"] == 443
    assert [r["CidrIp"] for r in ingress["IpRanges"]] == ["10.0.0.0/16", "192.168.0.0/24"]


async def test_delete_security_group_then_describe_confirms_gone(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    group_id = await _create_sg(stores, sink, ec2, vpc_id)
...
async def test_delete_vpc_blocked_by_subnet_then_sg_then_succeeds(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    subnet_req = sink.call(lambda: ec2.create_subnet(VpcId=vpc_id, CidrBlock="10.0.1.0/24"))
    subnet_id = _parse("CreateSubnet", await _answer(stores, subnet_req))["Subnet"]["SubnetId"]
    group_id = await _create_sg(stores, sink, ec2, vpc_id)

    blocked = await _answer(stores, sink.call(lambda: ec2.delete_vpc(VpcId=vpc_id)))
    assert blocked.status_code == 400
    assert _parse("DeleteVpc", blocked, error=True)["Error"]["Code"] == "DependencyViolation"

    await _answer(stores, sink.call(lambda: ec2.delete_subnet(SubnetId=subnet_id)))
    still_blocked = await _answer(stores, sink.call(lambda: ec2.delete_vpc(VpcId=vpc_id)))
    assert _parse("DeleteVpc", still_blocked, error=True)["Error"]["Code"] == "DependencyViolation"

    await _answer(stores, sink.call(lambda: ec2.delete_security_group(GroupId=group_id)))
    assert (await _answer(stores, sink.call(lambda: ec2.delete_vpc(VpcId=vpc_id)))).status_code == 200

    confirm = await _answer(stores, sink.call(lambda: ec2.describe_vpcs(VpcIds=[vpc_id])))
    assert _parse("DescribeVpcs", confirm, error=True)["Error"]["Code"] == "InvalidVpcID.NotFound"


async def test_delete_vpc_removes_its_default_security_group(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    await _answer(stores, sink.call(lambda: ec2.delete_vpc(VpcId=vpc_id)))
    req = sink.call(lambda: ec2.describe_security_groups(
        Filters=[{"Name": "vpc-id", "Values": [vpc_id]}],
    ))
    assert _parse("DescribeSecurityGroups", await _answer(stores, req))["SecurityGroups"] == []


async def test_delete_last_vpc_drops_the_envs_nebula_ca(sink, ec2, stores):
    ...
    assert (await _answer(stores, sink.call(lambda: ec2.delete_vpc(VpcId=vpc_a)))).status_code == 200
    assert nebula_dir.exists()

    assert (await _answer(stores, sink.call(lambda: ec2.delete_vpc(VpcId=vpc_b)))).status_code == 200
    assert not nebula_dir.exists()


async def test_delete_last_vpc_stops_the_envs_lighthouse_before_deleting_its_config(sink, ec2, stores, monkeypatch):
...
async def test_purge_env_forgets_the_network_records_an_interrupted_apply_stranded(sink, ec2, stores, monkeypatch):
    ...
    vpc_id = await _create_vpc(stores, sink, ec2)
    await _create_sg(stores, sink, ec2, vpc_id, name="web")
    req = sink.call(lambda: ec2.create_subnet(VpcId=vpc_id, CidrBlock="10.0.1.0/24"))
    _parse("CreateSubnet", await _answer(stores, req))

    forgotten = await ec2net_mod.purge_env(stores, ENV)
    ...
    assert await ec2net_mod.purge_env(stores, ENV) == []


# --- Tags (EC2's own wire shape) ------------------------------------------------


async def test_create_tags_describe_tags_round_trip(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    ...
    parsed = _parse("DescribeTags", await _answer(stores, describe_req))
...
async def test_create_tags_can_tag_multiple_resources_at_once(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    subnet_req = sink.call(lambda: ec2.create_subnet(VpcId=vpc_id, CidrBlock="10.0.1.0/24"))
    subnet_id = _parse("CreateSubnet", await _answer(stores, subnet_req))["Subnet"]["SubnetId"]
...
async def test_delete_tags_keyed_removes_only_matching(sink, ec2, stores):
    ...
    await _answer(stores, delete_req)
    assert stores.tags.get(ENV, f"ec2:{vpc_id}") == {"team": "infra"}


async def test_delete_tags_without_keys_removes_all(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2)
    stores.tags.set(ENV, f"ec2:{vpc_id}", {"env": "prod", "team": "infra"})
...
async def test_tag_specification_at_create_shows_in_describes(sink, ec2, stores):
    vpc_id = await _create_vpc(stores, sink, ec2, TagSpecifications=[
        {"ResourceType": "vpc", "Tags": [{"Key": "Name", "Value": "main"}]},
    ])
    describe_req = sink.call(lambda: ec2.describe_vpcs(VpcIds=[vpc_id]))
    parsed = _parse("DescribeVpcs", await _answer(stores, describe_req))
...
async def test_describe_network_interfaces_is_always_empty(sink, ec2, stores):
    req = sink.call(lambda: ec2.describe_network_interfaces(
        Filters=[{"Name": "vpc-id", "Values": ["vpc-123"]}],
    ))
    parsed = _parse("DescribeNetworkInterfaces", await _answer(stores, req))
    assert parsed["NetworkInterfaces"] == []


async def test_unmodeled_ec2_action_gets_invalid_action_not_a_503(stores):
    ...
    response = await synth.pure_answer("ec2:DescribeVpcClassicLink", "*", ENV, body, stores, 0.0)
    assert response is not None and response.status_code == 400
    assert b"InvalidAction" in response.body


async def test_state_persists_at_the_ec2net_sidecar_path(sink, ec2, stores, tmp_path):
    await _create_vpc(stores, sink, ec2)
