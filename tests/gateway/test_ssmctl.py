...
every call routed through classify() -> synth.pure_answer().
"""
...
def _put(stores, sink, ssm, name=NAME, value=VALUE, param_type="SecureString", **kwargs) -> Response:
    return _answer(stores, sink.call(lambda: ssm.put_parameter(Name=name, Value=value, Type=param_type, **kwargs)))


def _get(stores, sink, ssm, name=NAME, **kwargs) -> Response:
    return _answer(stores, sink.call(lambda: ssm.get_parameter(Name=name, **kwargs)))


def _describe(stores, sink, ssm, **kwargs) -> Response:
    return _answer(stores, sink.call(lambda: ssm.describe_parameters(**kwargs)))


# --- write / read -----------------------------------------------------------


def test_put_parameter_returns_version_one_and_the_tier(stores, sink, ssm):
    parsed = _parse("PutParameter", _put(stores, sink, ssm))
...
def test_get_parameter_returns_the_value_arn_and_type(stores, sink, ssm):
    _put(stores, sink, ssm)
    parameter = _parse("GetParameter", _get(stores, sink, ssm, WithDecryption=True))["Parameter"]
...
def test_a_securestring_is_stored_and_read_exactly_like_a_string(stores, sink, ssm):
    ...
    _put(stores, sink, ssm, name="/a/secure", param_type="SecureString", value="same-bytes")
    _put(stores, sink, ssm, name="/a/plain", param_type="String", value="same-bytes")

    with_decryption = _parse("GetParameter", _get(stores, sink, ssm, name="/a/secure", WithDecryption=True))
    without = _parse("GetParameter", _get(stores, sink, ssm, name="/a/secure", WithDecryption=False))
...
def test_put_parameter_without_overwrite_refuses_to_clobber(stores, sink, ssm):
    _put(stores, sink, ssm)
    parsed = _parse("PutParameter", _put(stores, sink, ssm, value="other"), error=True)

    assert parsed["Error"]["Code"] == "ParameterAlreadyExists"
    assert ssmctl.parameter_value(stores, ENV, NAME) == VALUE


def test_overwrite_bumps_the_version(stores, sink, ssm):
    _put(stores, sink, ssm)
    parsed = _parse("PutParameter", _put(stores, sink, ssm, value="rotated", Overwrite=True))
    parameter = _parse("GetParameter", _get(stores, sink, ssm))["Parameter"]
...
def test_an_invalid_type_is_a_validation_exception(stores, sink, ssm):
...
def test_get_a_missing_parameter_is_parameter_not_found(stores, sink, ssm):
    parsed = _parse("GetParameter", _get(stores, sink, ssm), error=True)

    assert parsed["Error"]["Code"] == "ParameterNotFound"


def test_a_root_level_leading_slash_is_optional_both_ways(stores, sink, ssm):
    ...
    gates the other."""
    _put(stores, sink, ssm, name="db-url", value="postgres://x")
    parameter = _parse("GetParameter", _get(stores, sink, ssm, name="/db-url"))["Parameter"]
...
def test_get_parameters_splits_found_from_invalid(stores, sink, ssm):
    _put(stores, sink, ssm, name="/a/one", value="1")
    parsed = _parse(
        "GetParameters",
        _answer(stores, sink.call(lambda: ssm.get_parameters(Names=["/a/one", "/a/missing"], WithDecryption=True))),
    )
...
def test_get_parameters_by_path_is_one_level_unless_recursive(stores, sink, ssm):
    _put(stores, sink, ssm, name="/app/db", value="1")
    _put(stores, sink, ssm, name="/app/nested/key", value="2")
    _put(stores, sink, ssm, name="/other/x", value="3")

    shallow = _parse("GetParametersByPath", _answer(stores, sink.call(lambda: ssm.get_parameters_by_path(Path="/app"))))
    deep = _parse(
        "GetParametersByPath",
        _answer(stores, sink.call(lambda: ssm.get_parameters_by_path(Path="/app", Recursive=True))),
    )
...
def test_delete_parameter_removes_it_and_its_tags(stores, sink, ssm):
    _put(stores, sink, ssm, Tags=[{"Key": "odin:node", "Value": NAME}])
    _answer(stores, sink.call(lambda: ssm.delete_parameter(Name=NAME)))

    assert not ssmctl.parameter_exists(stores, ENV, NAME)
    assert stores.tags.get(ENV, f"ssm:{NAME}") == {}


def test_delete_a_missing_parameter_is_parameter_not_found(stores, sink, ssm):
    parsed = _parse("DeleteParameter", _answer(stores, sink.call(lambda: ssm.delete_parameter(Name=NAME))), error=True)
...
def test_tags_round_trip_from_put_through_list_and_removal(stores, sink, ssm):
    _put(stores, sink, ssm, Tags=[{"Key": "odin:node", "Value": NAME}])
    listed = _parse("ListTagsForResource", _answer(stores, sink.call(
        lambda: ssm.list_tags_for_resource(ResourceType="Parameter", ResourceId=NAME),
    )))
    _answer(stores, sink.call(lambda: ssm.add_tags_to_resource(
        ResourceType="Parameter", ResourceId=NAME, Tags=[{"Key": "env", "Value": "dev"}],
    )))
    added = _parse("ListTagsForResource", _answer(stores, sink.call(
        lambda: ssm.list_tags_for_resource(ResourceType="Parameter", ResourceId=NAME),
    )))
    _answer(stores, sink.call(lambda: ssm.remove_tags_from_resource(
        ResourceType="Parameter", ResourceId=NAME, TagKeys=["env"],
    )))
    removed = _parse("ListTagsForResource", _answer(stores, sink.call(
        lambda: ssm.list_tags_for_resource(ResourceType="Parameter", ResourceId=NAME),
    )))
...
def test_an_unmodeled_action_is_a_protocol_correct_error(stores, sink, ssm):
    _put(stores, sink, ssm)
    parsed = _parse(
        "LabelParameterVersion",
        _answer(stores, sink.call(lambda: ssm.label_parameter_version(Name=NAME, Labels=["prod"]))),
        error=True,
    )
