...
async def _answer(stores: SynthStores, req) -> Response:
    ...
    # `synth.pure_answer` is a coroutine (v0.7.7). The `sink.call(lambda: ...)`
    # captures above are NOT: those drive a real boto3 client, which is
    # synchronous -- awaiting one would be a TypeError, not a fix.
...
async def _create_group(stores, sink, logs, name=GROUP, **kwargs) -> Response:
    req = sink.call(lambda: logs.create_log_group(logGroupName=name, **kwargs))
    return await _answer(stores, req)


async def _create_stream(stores, sink, logs, group=GROUP, stream=STREAM) -> Response:
    req = sink.call(lambda: logs.create_log_stream(logGroupName=group, logStreamName=stream))
    return await _answer(stores, req)


async def _put(stores, sink, logs, events, group=GROUP, stream=STREAM) -> dict:
    req = sink.call(lambda: logs.put_log_events(logGroupName=group, logStreamName=stream, logEvents=events))
    return _parse("PutLogEvents", await _answer(stores, req))


# --- control plane: CreateLogGroup / Describe / Delete / retention -----------


async def test_create_then_describe_log_group_reports_the_wildcard_arn(sink, logs, stores):
    assert (await _create_group(stores, sink, logs)).status_code == 200
    req = sink.call(lambda: logs.describe_log_groups())
    (group,) = _parse("DescribeLogGroups", await _answer(stores, req))["logGroups"]
...
async def test_create_log_group_twice_is_already_exists(sink, logs, stores):
    await _create_group(stores, sink, logs)
    response = await _create_group(stores, sink, logs)
    assert response.status_code == 400
    assert _parse("CreateLogGroup", response, error=True)["Error"]["Code"] == "ResourceAlreadyExistsException"


async def test_create_log_group_adopts_a_substrate_auto_created_group(sink, logs, stores):
    ...
    assert (await _create_group(stores, sink, logs)).status_code == 400


async def test_describe_log_groups_filters_by_prefix(sink, logs, stores):
    await _create_group(stores, sink, logs, name="/aws/lambda/a")
    await _create_group(stores, sink, logs, name="/ecs/web")
    req = sink.call(lambda: logs.describe_log_groups(logGroupNamePrefix="/aws/"))
    parsed = _parse("DescribeLogGroups", await _answer(stores, req))
    assert [g["logGroupName"] for g in parsed["logGroups"]] == ["/aws/lambda/a"]


async def test_put_and_delete_retention_policy_round_trip(sink, logs, stores):
    await _create_group(stores, sink, logs)
    put = sink.call(lambda: logs.put_retention_policy(logGroupName=GROUP, retentionInDays=14))
    assert (await _answer(stores, put)).status_code == 200
    req = sink.call(lambda: logs.describe_log_groups())
    (group,) = _parse("DescribeLogGroups", await _answer(stores, req))["logGroups"]
    assert group["retentionInDays"] == 14

    delete = sink.call(lambda: logs.delete_retention_policy(logGroupName=GROUP))
    assert (await _answer(stores, delete)).status_code == 200
    req = sink.call(lambda: logs.describe_log_groups())
    (group,) = _parse("DescribeLogGroups", await _answer(stores, req))["logGroups"]
    assert "retentionInDays" not in group


async def test_retention_policy_on_unknown_group_is_not_found(sink, logs, stores):
...
async def test_delete_log_group_removes_streams_events_and_tags(sink, logs, stores):
    await _create_group(stores, sink, logs, tags={"team": "infra"})
    await _create_stream(stores, sink, logs)
    await _put(stores, sink, logs, [{"timestamp": 1000, "message": "hello"}])
...
async def test_delete_unknown_log_group_is_not_found(sink, logs, stores):
...
async def test_create_log_group_seeds_tags_and_list_tags_for_resource_echoes_them(sink, logs, stores):
    await _create_group(stores, sink, logs, tags={"env": "prod"})
    ...
    assert _parse("ListTagsForResource", await _answer(stores, req))["tags"] == {"env": "prod"}


async def test_tag_and_untag_resource_round_trip(sink, logs, stores):
    await _create_group(stores, sink, logs)
    arn = logsctl.group_arn(GROUP)
    tag = sink.call(lambda: logs.tag_resource(resourceArn=arn, tags={"team": "infra"}))
    assert (await _answer(stores, tag)).status_code == 200
    listed = sink.call(lambda: logs.list_tags_for_resource(resourceArn=arn))
    assert _parse("ListTagsForResource", await _answer(stores, listed))["tags"] == {"team": "infra"}

    untag = sink.call(lambda: logs.untag_resource(resourceArn=arn, tagKeys=["team"]))
    assert (await _answer(stores, untag)).status_code == 200
    listed = sink.call(lambda: logs.list_tags_for_resource(resourceArn=arn))
    assert _parse("ListTagsForResource", await _answer(stores, listed))["tags"] == {}


async def test_list_tags_accepts_the_wildcard_arn_form_too(sink, logs, stores):
    await _create_group(stores, sink, logs, tags={"a": "b"})
    req = sink.call(lambda: logs.list_tags_for_resource(resourceArn=f"{logsctl.group_arn(GROUP)}:*"))
    assert _parse("ListTagsForResource", await _answer(stores, req))["tags"] == {"a": "b"}


async def test_legacy_list_tags_log_group_answers_by_group_name(sink, logs, stores):
    await _create_group(stores, sink, logs, tags={"a": "b"})
    req = sink.call(lambda: logs.list_tags_log_group(logGroupName=GROUP))
    assert _parse("ListTagsLogGroup", await _answer(stores, req))["tags"] == {"a": "b"}


async def test_tag_resource_on_unknown_group_is_not_found(sink, logs, stores):
...
async def test_create_log_stream_then_describe_log_streams(sink, logs, stores):
    await _create_group(stores, sink, logs)
    assert (await _create_stream(stores, sink, logs)).status_code == 200
    req = sink.call(lambda: logs.describe_log_streams(logGroupName=GROUP))
    (stream,) = _parse("DescribeLogStreams", await _answer(stores, req))["logStreams"]
...
async def test_create_log_stream_twice_is_already_exists(sink, logs, stores):
    await _create_group(stores, sink, logs)
    await _create_stream(stores, sink, logs)
    response = await _create_stream(stores, sink, logs)
    assert _parse("CreateLogStream", response, error=True)["Error"]["Code"] == "ResourceAlreadyExistsException"


async def test_create_log_stream_without_its_group_is_not_found(sink, logs, stores):
    response = await _create_stream(stores, sink, logs)
    assert _parse("CreateLogStream", response, error=True)["Error"]["Code"] == "ResourceNotFoundException"


async def test_put_then_get_log_events_round_trips_messages_in_timestamp_order(sink, logs, stores):
    await _create_group(stores, sink, logs)
    await _create_stream(stores, sink, logs)
    put = await _put(stores, sink, logs, [
    ...
    parsed = _parse("GetLogEvents", await _answer(stores, req))
...
async def test_get_log_events_honors_start_and_end_time_and_limit(sink, logs, stores):
    await _create_group(stores, sink, logs)
    await _create_stream(stores, sink, logs)
    await _put(stores, sink, logs, [{"timestamp": t, "message": f"m{t}"} for t in (1000, 2000, 3000)])

    windowed = sink.call(lambda: logs.get_log_events(
        logGroupName=GROUP, logStreamName=STREAM, startTime=2000, endTime=3000))
    parsed = _parse("GetLogEvents", await _answer(stores, windowed))
    ...
    parsed = _parse("GetLogEvents", await _answer(stores, limited))
    ...
    parsed = _parse("GetLogEvents", await _answer(stores, from_head))
    assert [e["message"] for e in parsed["events"]] == ["m1000"]


async def test_get_log_events_next_forward_token_pages_and_then_terminates(sink, logs, stores):
    await _create_group(stores, sink, logs)
    await _create_stream(stores, sink, logs)
    await _put(stores, sink, logs, [{"timestamp": t, "message": f"m{t}"} for t in (1000, 2000)])

    first = sink.call(lambda: logs.get_log_events(
        logGroupName=GROUP, logStreamName=STREAM, limit=1, startFromHead=True))
    parsed = _parse("GetLogEvents", await _answer(stores, first))
    assert [e["message"] for e in parsed["events"]] == ["m1000"]
    ...
    parsed = _parse("GetLogEvents", await _answer(stores, second))
    assert [e["message"] for e in parsed["events"]] == ["m2000"]
    ...
    parsed = _parse("GetLogEvents", await _answer(stores, third))
    assert parsed["events"] == []  # a paginator stops here rather than looping


async def test_put_log_events_to_a_missing_group_or_stream_is_not_found(sink, logs, stores):
    ...
    response = await _answer(stores, missing_group)
    ...
    response = await _answer(stores, missing_stream)
    assert _parse("PutLogEvents", response, error=True)["Error"]["Code"] == "ResourceNotFoundException"


async def test_filter_log_events_across_streams_with_a_substring_pattern(sink, logs, stores):
    await _create_group(stores, sink, logs)
    await _create_stream(stores, sink, logs, stream="task-a")
    await _create_stream(stores, sink, logs, stream="task-b")
    await _put(stores, sink, logs, [{"timestamp": 1000, "message": "INFO started"}], stream="task-a")
    await _put(stores, sink, logs, [{"timestamp": 2000, "message": "ERROR boom"}], stream="task-b")

    every = sink.call(lambda: logs.filter_log_events(logGroupName=GROUP))
    parsed = _parse("FilterLogEvents", await _answer(stores, every))
    ...
    parsed = _parse("FilterLogEvents", await _answer(stores, matched))
    ...
    parsed = _parse("FilterLogEvents", await _answer(stores, by_stream))
    assert [e["message"] for e in parsed["events"]] == ["INFO started"]


async def test_filter_and_describe_on_an_unknown_group_are_not_found(sink, logs, stores):
    filtered = sink.call(lambda: logs.filter_log_events(logGroupName="/ghost"))
    assert _parse("FilterLogEvents", await _answer(stores, filtered), error=True)["Error"]["Code"] == "ResourceNotFoundException"
    described = sink.call(lambda: logs.describe_log_streams(logGroupName="/ghost"))
    assert _parse("DescribeLogStreams", await _answer(stores, described), error=True)["Error"]["Code"] == "ResourceNotFoundException"


async def test_describe_log_streams_orders_by_last_event_time_when_asked(sink, logs, stores):
    await _create_group(stores, sink, logs)
    await _create_stream(stores, sink, logs, stream="b-newer")
    await _create_stream(stores, sink, logs, stream="a-older")
    await _put(stores, sink, logs, [{"timestamp": 1000, "message": "old"}], stream="a-older")
    await _put(stores, sink, logs, [{"timestamp": 9000, "message": "new"}], stream="b-newer")

    by_name = sink.call(lambda: logs.describe_log_streams(logGroupName=GROUP))
    parsed = _parse("DescribeLogStreams", await _answer(stores, by_name))
    ...
    parsed = _parse("DescribeLogStreams", await _answer(stores, by_time))
...
async def test_ingest_auto_creates_group_and_stream_then_reads_back_over_the_wire(sink, logs, stores):
...
async def test_unmodeled_logs_action_gets_invalid_action_not_a_503(stores):
    response = await synth.pure_answer("logs:PutMetricFilter", "*", ENV, b"{}", stores, 0.0)
    assert response is not None and response.status_code == 400
    assert b"InvalidAction" in response.body


async def test_create_log_group_without_a_name_is_invalid_parameter(stores):
    response = await synth.pure_answer("logs:CreateLogGroup", "*", ENV, json.dumps({}).encode(), stores, 0.0)
    assert response.status_code == 400
...
async def test_a_request_that_named_no_log_group_says_so_instead_of_blaming_the_name(op, payload, expected, stores):
    response = await synth.pure_answer(f"logs:{op}", "*", ENV, json.dumps(payload).encode(), stores, 0.0)
...
async def test_the_group_identifier_alternates_are_ONE_identifier_not_two(stores, sink, logs):
    ...
    by_identifier = await synth.pure_answer(
...
async def test_state_persists_at_the_logsctl_sidecar_path(sink, logs, stores, tmp_path):
    await _create_group(stores, sink, logs)
