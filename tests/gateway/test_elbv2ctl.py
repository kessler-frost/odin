...
def _answer(stores, req, proxy) -> Response:
    ...
    return elbv2ctl.pure_answer(action, resource, ENV, req.body, stores, time.monotonic(), proxy=proxy)


def _create_lb(stores, sink, elbv2, proxy, **kwargs) -> dict:
    kwargs.setdefault("Name", LB)
    kwargs.setdefault("Subnets", [SUBNET])
    req = sink.call(lambda: elbv2.create_load_balancer(**kwargs))
    (lb,) = _parse("CreateLoadBalancer", _answer(stores, req, proxy))["LoadBalancers"]
    return lb


def _create_tg(stores, sink, elbv2, proxy, **kwargs) -> dict:
    ...
    (tg,) = _parse("CreateTargetGroup", _answer(stores, req, proxy))["TargetGroups"]
    return tg


def _create_listener(stores, sink, elbv2, proxy, lb_arn: str, tg_arn: str, port: int = 80) -> dict:
    req = sink.call(lambda: elbv2.create_listener(
        LoadBalancerArn=lb_arn, Protocol="HTTP", Port=port,
        DefaultActions=[{"Type": "forward", "TargetGroupArn": tg_arn}],
    ))
    (listener,) = _parse("CreateListener", _answer(stores, req, proxy))["Listeners"]
    return listener


def _trio(stores, sink, elbv2, proxy) -> tuple[dict, dict, dict]:
    """The three resources ONE `alb` canvas node expands to."""
    lb = _create_lb(stores, sink, elbv2, proxy)
    tg = _create_tg(stores, sink, elbv2, proxy)
    listener = _create_listener(stores, sink, elbv2, proxy, lb["LoadBalancerArn"], tg["TargetGroupArn"])
...
def test_create_load_balancer_answers_provisioning_and_brings_up_the_real_proxy(stores, sink, elbv2, proxy):
    lb = _create_lb(stores, sink, elbv2, proxy, Tags=[{"Key": "odin:node", "Value": LB}])
    assert lb["LoadBalancerName"] == LB
    assert lb["Type"] == "application"
    # Honest asynchrony: the real nginx container comes up on a daemon thread,
    ...
    # `_spawn` runs the converge on a daemon thread -- poll rather than sleep.
    deadline = time.monotonic() + 2.0
    while time.monotonic() < deadline and not proxy.ensured:
        time.sleep(0.01)
    assert proxy.ensured, "CreateLoadBalancer must bring up the real proxy"


def test_describe_load_balancers_reports_active_once_the_proxy_is_up(stores, sink, elbv2, proxy):
    ...
    (lb,) = _parse("DescribeLoadBalancers", _answer(stores, req, proxy))["LoadBalancers"]
...
def test_a_second_load_balancer_with_the_same_name_is_rejected(stores, sink, elbv2, proxy):
    _create_lb(stores, sink, elbv2, proxy)
    req = sink.call(lambda: elbv2.create_load_balancer(Name=LB, Subnets=[SUBNET]))
    parsed = _parse("CreateLoadBalancer", _answer(stores, req, proxy), error=True)
    assert parsed["Error"]["Code"] == "DuplicateLoadBalancerName"


def test_describing_an_unknown_load_balancer_is_a_real_not_found(stores, sink, elbv2, proxy):
    req = sink.call(lambda: elbv2.describe_load_balancers(Names=["nope"]))
    parsed = _parse("DescribeLoadBalancers", _answer(stores, req, proxy), error=True)
    assert parsed["Error"]["Code"] == "LoadBalancerNotFound"


def test_delete_load_balancer_removes_its_listeners_and_the_real_container(stores, sink, elbv2, proxy):
    lb, _tg, _listener = _trio(stores, sink, elbv2, proxy)
    req = sink.call(lambda: elbv2.delete_load_balancer(LoadBalancerArn=lb["LoadBalancerArn"]))
    _parse("DeleteLoadBalancer", _answer(stores, req, proxy))
    ...
    assert _parse("DescribeLoadBalancers", _answer(stores, follow_up, proxy), error=True)["Error"]["Code"] == "LoadBalancerNotFound"


# --- attributes: where drift hides -----------------------------------------


def test_load_balancer_attributes_are_seeded_with_real_aws_defaults(stores, sink, elbv2, proxy):
    ...
    attributes = {a["Key"]: a["Value"] for a in _parse("DescribeLoadBalancerAttributes", _answer(stores, req, proxy))["Attributes"]}
...
def test_modifying_a_load_balancer_attribute_survives_the_next_read(stores, sink, elbv2, proxy):
    lb = _create_lb(stores, sink, elbv2, proxy)
    modify = sink.call(lambda: elbv2.modify_load_balancer_attributes(
        LoadBalancerArn=lb["LoadBalancerArn"],
        Attributes=[{"Key": "idle_timeout.timeout_seconds", "Value": "120"}],
    ))
    _parse("ModifyLoadBalancerAttributes", _answer(stores, modify, proxy))
    read = sink.call(lambda: elbv2.describe_load_balancer_attributes(LoadBalancerArn=lb["LoadBalancerArn"]))
    attributes = {a["Key"]: a["Value"] for a in _parse("DescribeLoadBalancerAttributes", _answer(stores, read, proxy))["Attributes"]}
    assert attributes["idle_timeout.timeout_seconds"] == "120"
    # The merge is a merge: the other defaults are untouched.
    assert attributes["routing.http2.enabled"] == "true"


def test_target_group_attributes_are_seeded_and_mergeable(stores, sink, elbv2, proxy):
    tg = _create_tg(stores, sink, elbv2, proxy)
    read = sink.call(lambda: elbv2.describe_target_group_attributes(TargetGroupArn=tg["TargetGroupArn"]))
    attributes = {a["Key"]: a["Value"] for a in _parse("DescribeTargetGroupAttributes", _answer(stores, read, proxy))["Attributes"]}
    ...
    _parse("ModifyTargetGroupAttributes", _answer(stores, modify, proxy))
    again = sink.call(lambda: elbv2.describe_target_group_attributes(TargetGroupArn=tg["TargetGroupArn"]))
    merged = {a["Key"]: a["Value"] for a in _parse("DescribeTargetGroupAttributes", _answer(stores, again, proxy))["Attributes"]}
    assert merged["deregistration_delay.timeout_seconds"] == "5"
    assert merged["stickiness.enabled"] == "false"


def test_listener_attributes_are_empty_for_an_application_listener(stores, sink, elbv2, proxy):
    ...
    _lb, _tg, listener = _trio(stores, sink, elbv2, proxy)
    req = sink.call(lambda: elbv2.describe_listener_attributes(ListenerArn=listener["ListenerArn"]))
    assert _parse("DescribeListenerAttributes", _answer(stores, req, proxy))["Attributes"] == []


def test_modifying_a_listener_attribute_survives_the_next_read(stores, sink, elbv2, proxy):
    _lb, _tg, listener = _trio(stores, sink, elbv2, proxy)
    modify = sink.call(lambda: elbv2.modify_listener_attributes(
        ListenerArn=listener["ListenerArn"],
        Attributes=[{"Key": "tcp.idle_timeout.seconds", "Value": "60"}],
    ))
    _parse("ModifyListenerAttributes", _answer(stores, modify, proxy))
    read = sink.call(lambda: elbv2.describe_listener_attributes(ListenerArn=listener["ListenerArn"]))
    assert _parse("DescribeListenerAttributes", _answer(stores, read, proxy))["Attributes"] == [
...
def test_create_time_tags_round_trip_through_describe_tags(stores, sink, elbv2, proxy):
    lb = _create_lb(stores, sink, elbv2, proxy, Tags=[{"Key": "odin:node", "Value": LB}])
    req = sink.call(lambda: elbv2.describe_tags(ResourceArns=[lb["LoadBalancerArn"]]))
    (description,) = _parse("DescribeTags", _answer(stores, req, proxy))["TagDescriptions"]
    assert description["ResourceArn"] == lb["LoadBalancerArn"]
    assert description["Tags"] == [{"Key": "odin:node", "Value": LB}]


def test_add_and_remove_tags_work_on_all_three_resource_kinds(stores, sink, elbv2, proxy):
    lb, tg, listener = _trio(stores, sink, elbv2, proxy)
    for arn in (lb["LoadBalancerArn"], tg["TargetGroupArn"], listener["ListenerArn"]):
        ...
        _parse("AddTags", _answer(stores, add, proxy))
        read = sink.call(lambda arn=arn: elbv2.describe_tags(ResourceArns=[arn]))
        (description,) = _parse("DescribeTags", _answer(stores, read, proxy))["TagDescriptions"]
        ...
        _parse("RemoveTags", _answer(stores, remove, proxy))
        after = sink.call(lambda arn=arn: elbv2.describe_tags(ResourceArns=[arn]))
        (description,) = _parse("DescribeTags", _answer(stores, after, proxy))["TagDescriptions"]
        assert "team" not in {t["Key"] for t in description["Tags"]}


def test_tagging_an_unknown_arn_is_a_real_not_found(stores, sink, elbv2, proxy):
    ...
    assert _parse("DescribeTags", _answer(stores, req, proxy), error=True)["Error"]["Code"] == "ResourceNotFound"


# --- target group + listener ------------------------------------------------


def test_create_target_group_echoes_the_health_check_it_was_given(stores, sink, elbv2, proxy):
    tg = _create_tg(stores, sink, elbv2, proxy, HealthCheckPath="/healthz", HealthCheckIntervalSeconds=10)
...
def test_a_target_group_reports_the_load_balancers_whose_listeners_forward_to_it(stores, sink, elbv2, proxy):
    lb, tg, _listener = _trio(stores, sink, elbv2, proxy)
...
def test_modify_target_group_updates_the_health_check_and_the_proxy_fail_window(stores, sink, elbv2, proxy):
    _lb, tg, _listener = _trio(stores, sink, elbv2, proxy)
    ...
    (modified,) = _parse("ModifyTargetGroup", _answer(stores, req, proxy))["TargetGroups"]
    ...
    deadline = time.monotonic() + 2.0
    while time.monotonic() < deadline and proxy.last_listeners[0].fail_timeout_seconds != 7:
        time.sleep(0.01)
    assert proxy.last_listeners[0].fail_timeout_seconds == 7


def test_create_listener_echoes_its_default_action_verbatim(stores, sink, elbv2, proxy):
    _lb, tg, listener = _trio(stores, sink, elbv2, proxy)
...
def test_an_unmodeled_default_action_type_is_refused_not_silently_accepted(stores, sink, elbv2, proxy):
    lb = _create_lb(stores, sink, elbv2, proxy)
    req = sink.call(lambda: elbv2.create_listener(
    ...
    parsed = _parse("CreateListener", _answer(stores, req, proxy), error=True)
    assert parsed["Error"]["Code"] == "ValidationError"
    assert "fixed-response" in parsed["Error"]["Message"]


def test_the_listener_set_is_what_the_proxy_listens_on(stores, sink, elbv2, proxy):
    lb, tg, _listener = _trio(stores, sink, elbv2, proxy)
    _create_listener(stores, sink, elbv2, proxy, lb["LoadBalancerArn"], tg["TargetGroupArn"], port=8080)
    deadline = time.monotonic() + 2.0
    while time.monotonic() < deadline and len(proxy.last_listeners) != 2:
        time.sleep(0.01)
    assert sorted(listener.port for listener in proxy.last_listeners) == [80, 8080]


def test_delete_listener_takes_it_out_of_the_proxy(stores, sink, elbv2, proxy):
    _lb, _tg, listener = _trio(stores, sink, elbv2, proxy)
    req = sink.call(lambda: elbv2.delete_listener(ListenerArn=listener["ListenerArn"]))
    _parse("DeleteListener", _answer(stores, req, proxy))
    deadline = time.monotonic() + 2.0
    while time.monotonic() < deadline and proxy.last_listeners:
        time.sleep(0.01)
...
def test_register_targets_becomes_the_proxys_upstream_list(stores, sink, elbv2, proxy):
    _lb, tg, _listener = _trio(stores, sink, elbv2, proxy)
    req = sink.call(lambda: elbv2.register_targets(
        TargetGroupArn=tg["TargetGroupArn"],
        Targets=[{"Id": CONTAINER_HOST, "Port": 32768}, {"Id": CONTAINER_HOST, "Port": 32769}],
    ))
    _parse("RegisterTargets", _answer(stores, req, proxy))
    deadline = time.monotonic() + 2.0
    while time.monotonic() < deadline and len(proxy.last_listeners[0].targets) != 2:
        time.sleep(0.01)
    assert proxy.last_listeners[0].targets == (f"{CONTAINER_HOST}:32768", f"{CONTAINER_HOST}:32769")


def test_deregister_targets_takes_one_out_again(stores, sink, elbv2, proxy):
    _lb, tg, _listener = _trio(stores, sink, elbv2, proxy)
    elbv2ctl.register_target(stores, ENV, tg["TargetGroupArn"], CONTAINER_HOST, 32768, proxy=proxy)
    elbv2ctl.register_target(stores, ENV, tg["TargetGroupArn"], CONTAINER_HOST, 32769, proxy=proxy)
    req = sink.call(lambda: elbv2.deregister_targets(
        TargetGroupArn=tg["TargetGroupArn"], Targets=[{"Id": CONTAINER_HOST, "Port": 32768}],
    ))
    _parse("DeregisterTargets", _answer(stores, req, proxy))
    deadline = time.monotonic() + 2.0
    while time.monotonic() < deadline and len(proxy.last_listeners[0].targets) != 1:
        time.sleep(0.01)
    assert proxy.last_listeners[0].targets == (f"{CONTAINER_HOST}:32769",)


def test_registering_the_same_target_twice_is_idempotent(stores, sink, elbv2, proxy):
    _lb, tg, _listener = _trio(stores, sink, elbv2, proxy)
    elbv2ctl.register_target(stores, ENV, tg["TargetGroupArn"], CONTAINER_HOST, 32768, proxy=proxy)
    elbv2ctl.register_target(stores, ENV, tg["TargetGroupArn"], CONTAINER_HOST, 32768, proxy=proxy)
...
def test_an_ec2_target_resolves_to_the_vms_real_address(stores, sink, elbv2, proxy):
    _lb, tg, _listener = _trio(stores, sink, elbv2, proxy)
    stores.ec2compute.set(ENV, "instance:i-0abc", {"instance_id": "i-0abc", "private_ip_address": "192.168.5.7"})
    elbv2ctl.register_target(stores, ENV, tg["TargetGroupArn"], "i-0abc", 8080, proxy=proxy)
    assert proxy.last_listeners[0].targets == ("192.168.5.7:8080",)


def test_describe_target_health_probes_for_real_and_never_invents_healthy(stores, sink, elbv2, proxy):
    ...
    elbv2ctl.register_target(stores, ENV, tg["TargetGroupArn"], "127.0.0.1", 1, proxy=proxy)
    req = sink.call(lambda: elbv2.describe_target_health(TargetGroupArn=tg["TargetGroupArn"]))
    (description,) = _parse("DescribeTargetHealth", _answer(stores, req, proxy))["TargetHealthDescriptions"]
...
def test_describe_target_health_reports_unused_when_health_checks_are_disabled(stores, sink, elbv2, proxy):
    _lb, tg, _listener = _trio(stores, sink, elbv2, proxy)
    elbv2ctl.register_target(stores, ENV, tg["TargetGroupArn"], "127.0.0.1", 1, proxy=proxy)
    modify = sink.call(lambda: elbv2.modify_target_group(
        TargetGroupArn=tg["TargetGroupArn"], HealthCheckEnabled=False,
    ))
    _parse("ModifyTargetGroup", _answer(stores, modify, proxy))
    req = sink.call(lambda: elbv2.describe_target_health(TargetGroupArn=tg["TargetGroupArn"]))
    (description,) = _parse("DescribeTargetHealth", _answer(stores, req, proxy))["TargetHealthDescriptions"]
    assert description["TargetHealth"]["State"] == "unused"


def test_registering_into_an_unknown_target_group_is_a_real_not_found(stores, sink, elbv2, proxy):
