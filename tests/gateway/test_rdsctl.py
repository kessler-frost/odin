...
class FakePostgresRds:
    ...
    database -- deterministic and instant, so the background create task's
    transitions can be observed with a short poll instead of a real Postgres
    boot.

    Every method the real class made a coroutine in v0.7.7 is `async def` here
    too, and `container_name` stays sync exactly as it is on `PostgresRds` --
    a fake that answered synchronously where the real one awaits would prove
    the state machine against a substrate odin does not have.
    ...
    async def join_mesh(self, db_id: str, firewall=None, revision: str = "") -> str | None:
    ...
    async def set_password(self, db_id: str, user: str, current: str, new: str) -> None:
...
async def _fake_probe(host, port, user, password, db="postgres") -> PgReady:
    ...
    Deliberately still a plain `def`: `rdsctl._wait_available` calls
    `pg_ready_sync` WITHOUT awaiting it (it is the sync probe beside
    `assertions.pg_ready`), so a coroutine here would be a truthy object whose
    `.ok` never existed."""
...
async def _call(body: bytes, stores: SynthStores, rds, action: str, resource: str = DB) -> Response:
    return await rdsctl.pure_answer(action, resource, ENV, body, stores, time.monotonic(), rds=rds)
...
def _captured(sink, client, call) -> tuple[str, str, bytes]:
    ...
    Stays SYNC, and the `lambda` with it: `client` is a boto3 client, whose
    methods are synchronous no matter how much of odin went async."""
...
async def _await_status(sink, client, stores, rds, status: str, timeout: float = 5.0) -> dict:
    """Poll DescribeDBInstances until the background create/delete task has
    landed `status`.

    `await asyncio.sleep`, never `time.sleep`: the work being waited for is now
    a task on THIS event loop (v0.7.7), so a blocking sleep here would freeze
    the very boot the poll is waiting for and the loop would spin to the
    deadline every time."""
...
async def test_mark_instance_failed_then_converge_recreates_the_container(tmp_path, sink, rds):
    ...
    # Still a plain `def` in v0.7.7 -- it starts tasks and awaits none of them.
...
async def test_converge_leaves_available_and_creating_instances_alone(tmp_path, sink, rds):
    ...
    await _create(sink, rds, stores, fake)
    await _await_status(sink, rds, stores, fake, "available")
    assert rdsctl.converge_db_instances(stores, ENV, substrate=fake) == []
    assert rdsctl.converge_db_instances(stores, ENV, substrate=fake) == []
    assert fake.created == [(DB, USER, PASSWORD, "postgres")]
...
async def test_the_database_joins_the_mesh_behind_its_assigned_sgs_compiled_firewall(tmp_path, sink, rds):
    ...
    # `_finish_create` writes `available` and then joins the mesh with no
    # `await` in between that suspends, so a poll that sees `available` has
    # necessarily seen the join too -- on one event loop that is a guarantee,
    # where the daemon thread this replaced only made it likely.
...
async def test_ensure_db_mesh_repushes_an_edited_sgs_rules_and_ignores_failed_instances(tmp_path, sink, rds):
    ...
    await rdsctl.ensure_db_mesh(stores, ENV, substrate=fake)
...
async def test_an_unmodeled_action_is_a_protocol_correct_invalid_action(tmp_path):
...
async def test_wait_for_available_instances_reports_a_database_that_never_came_back(tmp_path, sink, rds):
    ...
    returns, so /apply-full scored `applied` the instant the task was spawned
    even when Postgres never accepted a connection. The wait AWAITS that task
    ...
    faults = await rdsctl.wait_for_available_instances(stores, ENV, booting)
...
async def test_wait_for_available_instances_is_one_store_read_when_everything_is_available(tmp_path, sink, rds):
    ...
    faults = await rdsctl.wait_for_available_instances(stores, ENV, [])
...
async def test_wait_for_available_instances_waits_for_a_database_that_is_still_creating(tmp_path, sink, rds):
    ...
    await _create(sink, rds, stores, fake)
    # The create task is genuinely in flight: `_wait_available` polls until
    # the substrate says the container publishes a port.
    assert rdsctl.records(stores, ENV)[0]["status"] == "creating"

    # `loop.call_later`, not `threading.Timer`: the port has to appear on the
    # SAME loop the create task polls from, at the same 0.05s it always did.
    asyncio.get_running_loop().call_later(0.05, fake.up.add, DB)
    faults = await rdsctl.wait_for_available_instances(stores, ENV, [], timeout=5.0)
...
def test_available_timeout_defaults_to_outlasting_the_boot_it_verifies(monkeypatch):
    ...
    or the verification would hard-stop while the task it is verifying is
...
async def test_a_request_that_named_no_db_instance_says_so_instead_of_blaming_the_name(
    op, body, expected, tmp_path,
):
    stores = _stores(tmp_path)
    response = await rdsctl.pure_answer(f"rds:{op}", "", ENV, body, stores, 0.0, rds=FakePostgresRds())
...
async def test_a_named_instance_that_is_missing_still_gets_the_real_not_found_code(tmp_path):
