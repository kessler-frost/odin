...
async def test_ordinary_container_gets_the_host_gateway_alias():
    runner = FakeRunner()
    await ColimaRuntime(runner=runner).run_container(
        ContainerSpec(name="pg", image="postgres:16-alpine", ports={5432: 0}))
    call = _run_call(runner)
    assert "--add-host" in call and "host.docker.internal:host-gateway" in call
    assert "--network" not in call


async def test_namespace_sharing_sidecar_gets_tun_capabilities_and_no_add_host():
...
async def test_logs_asks_for_timestamps_so_the_two_streams_can_be_merged():
    runner = LogRunner()
    await ColimaRuntime(runner=runner).logs("pg", tail=10)
    assert runner.calls == [["docker", "logs", "--timestamps", "--tail", "10", "pg"]]


async def test_logs_returns_the_stderr_stream_a_settled_postgres_only_writes_to():
    ...
    assert await ColimaRuntime(runner=runner).logs("pg") == (
        "LOG:  database system is ready\nLOG:  checkpoint starting"
    )


async def test_logs_merges_the_two_streams_in_the_runtimes_own_timestamp_order():
    runner = LogRunner(
        stdout=(
    ...
    assert (await ColimaRuntime(runner=runner).logs("app")).splitlines() == [
        "out-first", "err-second", "out-third", "err-fourth",
    ]


async def test_the_tail_is_applied_after_combining_so_tail_n_means_n_real_lines():
    ...
    lines = (await ColimaRuntime(runner=runner).logs("app", tail=3)).splitlines()
    assert lines == ["err-2", "out-3", "err-4"]  # the last 3 of the 4 combined


async def test_a_line_the_runtime_never_stamped_is_kept_not_dropped():
...
async def test_a_postgres_lines_own_leading_date_is_never_mistaken_for_the_stamp():
    ...
    assert await ColimaRuntime(runner=runner).logs("pg") == (
        "2026-07-25 14:00:01.123 UTC [1] LOG:  ready"
    )
...
async def test_a_failed_command_with_no_named_container_still_names_the_subcommand():
    async def runner(args, input=None):
        return _Proc(1, "", "no such directory")

    with pytest.raises(RuntimeError, match=r"^docker cp failed \(exit 1\): no such directory$"):
        await ColimaRuntime(runner=runner).copy_in("job", "/host/odin.conf", "/etc/nginx/odin.conf")
...
async def test_a_failure_with_no_output_at_all_still_states_a_reason():
    ...
    with pytest.raises(RuntimeError) as raised:
        await ColimaRuntime(runner=runner).run_container(
            ContainerSpec(name="x", image="alpine:latest"))
    message = str(raised.value)
    assert message == (
...
async def test_a_failure_that_explained_itself_on_stdout_is_not_reported_as_silent():
    ...
    with pytest.raises(RuntimeError) as raised:
        await ColimaRuntime(runner=runner).copy_in("job", "/host/f", "/etc/f")
    assert str(raised.value) == "docker cp failed (exit 7): nothing on stderr; on stdout: on-stdout"


async def test_an_unreadable_port_map_names_the_exit_code_and_the_silence_too():
    ...
    with pytest.raises(PortUnreadable) as raised:
        await ColimaRuntime(runner=runner).host_port("gone", 8080)
    message = str(raised.value)
    assert message == (
...
async def test_a_failed_log_read_is_empty_not_the_clis_own_error_text():
    ...
    assert await ColimaRuntime(runner=runner).logs("gone") == ""
...
async def test_host_port_reads_the_structured_port_map_not_a_text_line():
    runner = PortRunner(stdout=_RUNNING)
    assert await ColimaRuntime(runner=runner).host_port("odin-aws-rustfs-prod", 9000) == 33776
    assert runner.calls == [
        ["docker", "inspect", "-f", "{{json .NetworkSettings.Ports}}", "odin-aws-rustfs-prod"],
    ]


async def test_a_port_the_container_publishes_nothing_on_is_an_honest_zero():
    # rc 0 means the runtime ANSWERED; "9001/tcp": null is that answer.
    assert await ColimaRuntime(runner=PortRunner(stdout=_RUNNING)).host_port("rustfs", 9001) == 0
    # An exited container's whole map is `{}` -- same shape of answer.
    assert await ColimaRuntime(runner=PortRunner(stdout="{}")).host_port("dead", 9000) == 0


async def test_a_port_read_that_FAILED_raises_instead_of_returning_zero():
    ...
    with pytest.raises(PortUnreadable, match="cannot read gone's published ports"):
        await ColimaRuntime(runner=runner).host_port("gone", 9000)


async def test_facts_never_asks_an_absent_container_for_a_port():
