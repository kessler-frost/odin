...
class FakeRuntime:
    ...
    async def signal(self, name, sig):
    ...
    async def start_target(self, name):
...
async def test_disabled_without_a_nebula_network(tmp_path):
    ...
    assert await mesh.ensure(TARGET, MEMBER) is None
...
async def test_ensure_joins_the_backing_via_a_shared_network_namespace(tmp_path):
    ...
    ip = await mesh.ensure(TARGET, MEMBER, firewall=DB_FIREWALL)
...
async def test_overlay_ip_is_sticky_and_readable_without_side_effects(tmp_path):
    ...
    assert await mesh.ensure(TARGET, MEMBER) == first  # re-join keeps the address
...
async def test_changed_firewall_reloads_the_sidecar_in_place(tmp_path):
    ...
    await mesh.ensure(TARGET, MEMBER, firewall=widened)
...
async def test_a_change_outside_the_firewall_still_replaces_the_daemon(tmp_path):
...
async def test_the_membership_revision_rides_in_the_firewall_block_and_reloads(tmp_path):
    ...
    await mesh.ensure(TARGET, MEMBER, firewall=DB_FIREWALL, revision="rev-two")
    assert runtime.signals == [(f"{TARGET}-mesh", "HUP")]
...
async def test_no_revision_renders_no_key_at_all(tmp_path):
...
async def test_a_replaced_target_container_gets_a_fresh_sidecar(tmp_path):
    ...
    await runtime.stop(TARGET)          # `docker kill` + the converge's own cleanup
    await runtime.start_target(TARGET)  # ...and the recreated database: a NEW id
    assert await mesh.attached_to(TARGET) is False, "the sidecar is in the dead container's namespace"
...
async def test_an_unchanged_target_does_not_churn_the_sidecar(tmp_path):
...
async def test_attached_to_is_unknown_rather_than_false_when_the_target_is_gone(tmp_path):
    ...
    mesh = _sidecar(tmp_path, runtime)
    await mesh.ensure(TARGET, MEMBER, firewall=DB_FIREWALL)
    assert await mesh.attached_to(TARGET) is None
    await mesh.ensure(TARGET, MEMBER, firewall=DB_FIREWALL)
    assert len(runtime.specs) == 1
...
async def test_a_dead_sidecar_is_restarted_even_with_an_unchanged_config(tmp_path):
    ...
    mesh = _sidecar(tmp_path, runtime)
    await mesh.ensure(TARGET, MEMBER, firewall=DB_FIREWALL)
    await runtime.stop(f"{TARGET}-mesh")  # crashed / host rebooted
    await mesh.ensure(TARGET, MEMBER, firewall=DB_FIREWALL)
    assert len(runtime.specs) == 2


async def test_certs_are_signed_once_per_member(tmp_path):
...
async def test_a_changed_membership_re_issues_the_cert_and_restarts_the_sidecar(tmp_path):
    ...
    await mesh.ensure(TARGET, MEMBER, groups=("sg-db",), firewall=DB_FIREWALL)
    signed = [c for c in runner.calls if "sign" in c and MEMBER in c]
    assert signed[0][signed[0].index("-groups") + 1] == "backing,sg-db"

    await mesh.ensure(TARGET, MEMBER, groups=("sg-internal",), firewall=DB_FIREWALL)
...
async def test_an_unchanged_membership_does_not_re_issue_or_restart(tmp_path):
    ...
    mesh = _sidecar(tmp_path, runtime, runner=runner)
    for _ in range(3):
        await mesh.ensure(TARGET, MEMBER, groups=("sg-db", "sg-ops"), firewall=DB_FIREWALL)
    # ...and neither does a reorder of the same groups.
    await mesh.ensure(TARGET, MEMBER, groups=("sg-ops", "sg-db"), firewall=DB_FIREWALL)
    assert len([c for c in runner.calls if "sign" in c and MEMBER in c]) == 1
    assert len(runtime.specs) == 1


async def test_a_restarted_sidecar_pokes_its_peers_to_re_handshake(tmp_path):
