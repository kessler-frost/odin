  ...
  **v1 limits, recorded rather than hidden** (northstar directive 5's honesty
  ...
  - Security groups: what IS enforced (W2.6) and what is not.
    ...
    - **RDS**: the Postgres container is a REAL mesh member (a nebula
      ...
      in `securityGroups`. A VM in `web-sg` reaches it; one that isn't is
    ...
    - **RESIDUAL GAP, stated plainly: the raw host port is still open and
      ...
      it, the reconciler probes it, host-side clients (tests, psql, boto3)
      use it, and `${{node.VAR}}` facts publish it. So anything that can
...
- **Recorded as UNSUPPORTED for now** (northstar directive 5's honesty rule):
  ALB/ELBv2, EKS, CloudFormation, autoscaling, KMS (the `kms` catalog node is
  an unbacked placeholder — no substitute, no gateway model, and as of W2.6 it
  advertises no IAM actions either, since a `kms:Encrypt` permission could
  never be enforced or even reached), and RDS-via-Terraform (rds nodes stay on
